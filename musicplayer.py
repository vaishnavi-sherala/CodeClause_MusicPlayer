...
statusbar = ttk.Label(root, text="Welcome to Vaishnavi Music Player", anchor=W, font='Arial 8 italic')
statusbar.pack(side=BOTTOM, fill=X)
statusbar1 = ttk.Label(root, text="Welcome to Vaishnavi Music Player", anchor=W, font='Arial 8 italic')
...
root.title("Vaishnavi Music Player")
...
rtframe = Frame(root,bg="#364C68")
rtframe.pack(pady=30,padx=20)
topframe = Frame(rtframe,bg="#364C68")
topframe.pack()
root.style = ttk.Style()
#root.style.theme_use("clam")
root.style.configure('TLabel', background='#364C68')
...
def play_music():
    ...
    if paused:
        ...
        try:
            ...
            tkinter.messagebox.showerror('File not found', 'Vaishnavi music player could not find the file. Please select again.')
def stop_music():
    mixer.music.stop()
